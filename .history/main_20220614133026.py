...
def post_login():
    ...
    if form.save.data and Usuarios.check_credentials(form.email.data, form.password.data):
        return redirect('/')    
    elif form.save.data and not Usuarios.check_user(form.email.data):
        print("error email")
        return redirect('login', form=form, error="El email no existe.")
    elif form.save.data and Usuarios.check_user(form.email.data) and Usuarios.get_password(form.email.data):
